...
from pipeline.common.utils import is_url
from pipeline.datasets.data_utils import concat_datasets, reorg_datasets_by_split, ChainDataset
from pipeline.datasets.datasets.dataloader_utils import (
    IterLoader,
    MultiIterLoader,
    PrefetchLoader,
)
...
class RunnerBase:
    ...
    def train(self):
        ...
        val_json_file = None
        if hasattr(self.config.run_cfg, "val_json_file"):
            val_json_file = self.config.run_cfg.val_json_file
        test_json_file = None
        if hasattr(self.config.run_cfg, "test_json_file"):
            test_json_file = self.config.run_cfg.test_json_file
        ...
        for cur_epoch in range(self.start_epoch, self.max_epoch):
            ...
            save_path = self._save_checkpoint(cur_epoch, is_best=False)
        ...
        test_logs = self.evaluate(cur_epoch=test_epoch, skip_reload=self.evaluate_only, file=test_json_file)
        for split_name, test_log in test_logs.items():
    ...
    def eval_epoch(self, split_name, cur_epoch, skip_reload=False, save_path=None, file=None):
        ...
        test_yaml_config = None
        if hasattr(self.config.run_cfg, "test_yaml_config"):
            test_yaml_config = self.config.run_cfg.test_yaml_config
